...
<template>
  <div :class="{ 'is-loading': isLoading }">
    <sba-alert
      v-if="error"
      :error="error"
      :title="$t('instances.gateway.filters.fetch_failed')"
    />
...
export default {
  ...
  computed: {
    globalFilters() {
      ...
      const filtered = this.$data._globalFilters.filter((globalFilter) =>
  ...
  methods: {
    async fetchGlobalFilters() {
      ...
      try {
        ...
        this.$data._globalFilters = Object.entries(response.data).map(
          ([name, order]) => {
